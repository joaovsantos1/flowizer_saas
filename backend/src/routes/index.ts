...
router.post('/webhooks/stripe', webhookRateLimit, webhookController.stripeWebhook);
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!parsed.success) {
      res.status(400).json({ error: 'Parâmetros inválidos', details: parsed.error.flatten().fieldErrors });
      return;
    }
    ...
    const query = db('transactions')
      ...
      .select('id', 'type', 'amount_cents', 'category', 'description_plain', 'transaction_date', 'created_at')
    ...
    const [transactions, countResult] = await Promise.all([
      query,
      db('transactions').where({ user_id: userId }).whereNull('deleted_at').count('id as count').first(),
    ]);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const { encrypt } = await import('../services/encryption/EncryptionService.js');
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    ...
    logAudit({ userId, action: 'transaction_deleted', resourceType: 'transaction', resourceId: id });
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    const reminders = await db('reminders')
      ...
      .select('id', 'title', 'scheduled_at', 'recurrence', 'status', 'next_run_at', 'recurrence_until')
  ...
  async (req: Request, res: Response) => {
    ...
    const [summary, byCategory] = await Promise.all([
      db('transactions')
        ...
        .whereNull('deleted_at')
        .select(
          db.raw("SUM(CASE WHEN type = 'income' THEN amount_cents ELSE 0 END) as total_income"),
          db.raw("SUM(CASE WHEN type = 'expense' THEN amount_cents ELSE 0 END) as total_expense"),
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { stripeService } = await import('../services/stripe/StripeService.js');
      const returnUrl = req.body.returnUrl ?? `${config.APP_URL}/profile`;
      const url = await stripeService.createPortalSession(req.user!.sub, returnUrl);
      res.json({ url });
    } catch (error) {
      if (config.NODE_ENV !== 'production') {
        res.status(503).json({
          error: 'Portal Stripe indisponível em desenvolvimento. Configure STRIPE_SECRET_KEY real para usar.',
          devMode: true,
        });
        return;
      }
...
router.get(
  ...
  async (_req, res: Response) => {
    const users = await db('users')
      .whereNull('deleted_at')
      .select('id', 'name', 'role', 'subscription_status', 'status', 'created_at')
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!parsed.success) { res.status(400).json({ error: 'Dados inválidos', details: parsed.error.flatten().fieldErrors }); return; }
      const { title, scheduledAt, message, recurrence } = parsed.data;
      const userId = req.user!.sub;
      const scheduledDate = new Date(scheduledAt);
      if (scheduledDate < new Date()) { res.status(400).json({ error: 'Data não pode ser no passado' }); return; }
      const maxDate = new Date(); maxDate.setFullYear(maxDate.getFullYear() + 1);
      if (scheduledDate > maxDate) { res.status(400).json({ error: 'Máximo 1 ano no futuro' }); return; }
      const { encrypt } = await import('../services/encryption/EncryptionService.js');
      const [reminder] = await db('reminders').insert({
        ...
        recurrence_until: parsed.data.recurrenceUntil ? new Date(parsed.data.recurrenceUntil) : null,
      }).returning(['id', 'title', 'scheduled_at', 'recurrence', 'status', 'recurrence_until']);
      logAudit({ userId, action: 'reminder_created', resourceType: 'reminder', resourceId: reminder?.id });
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { decrypt } = await import('../services/encryption/EncryptionService.js');
      const user = await db('users')
        .select('id','name','role','subscription_status','trial_ends_at','subscription_ends_at','created_at')
        .where({ id: req.user!.sub, deleted_at: null }).first();
...
router.patch(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { z } = await import('zod');
      const Schema = z.object({ name: z.string().min(2).max(100).trim().optional() });
      const parsed = Schema.safeParse(req.body);
      if (!parsed.success) { res.status(400).json({ error: 'Dados inválidos' }); return; }
      await db('users').where({ id: req.user!.sub }).update({ ...parsed.data, updated_at: new Date() });
...
router.patch(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { z } = await import('zod');
      const Schema = z.object({
        phone: z.string().regex(/^\+[1-9]\d{7,14}$/, 'Telefone deve estar no formato E.164: +5511999999999'),
        password: z.string().min(1),
      });
      const parsed = Schema.safeParse(req.body);
      if (!parsed.success) { res.status(400).json({ error: 'Dados inválidos', details: parsed.error.flatten().fieldErrors }); return; }

      const { verifyPassword, encrypt } = await import('../services/encryption/EncryptionService.js');
      const crypto = await import('crypto');

      // Verificar senha antes de alterar número
      const user = await db('users').select('password_hash').where({ id: req.user!.sub }).first();
      if (!user) { res.status(404).json({ error: 'Usuário não encontrado' }); return; }
      const valid = await verifyPassword(parsed.data.password, user.password_hash);
      if (!valid) { res.status(401).json({ error: 'Senha incorreta' }); return; }

      // Verificar se novo número já existe
      const newHash = crypto.createHash('sha256').update(parsed.data.phone).digest('hex');
      const existing = await db('users').where({ phone_hash: newHash }).whereNot({ id: req.user!.sub }).first();
      if (existing) { res.status(409).json({ error: 'Este número já está cadastrado em outra conta' }); return; }
...
router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!deleted) { res.status(404).json({ error: 'Lembrete não encontrado' }); return; }
      logAudit({ userId, action: 'reminder_deleted', resourceType: 'reminder', resourceId: id });
...
router.patch(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { verifyPassword, hashPassword } = await import('../services/encryption/EncryptionService.js');
      const user = await db('users').select('password_hash').where({ id: req.user!.sub }).first();
      if (!user) { res.status(404).json({ error: 'Usuário não encontrado' }); return; }
      ...
      await db('users').where({ id: req.user!.sub }).update({ password_hash: newHash });
